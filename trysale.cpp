...
public :
    ...
    void add_bill(int, int t_itemcode, char *t_itemname, float t_qty,
                  float t_cost, float t_price,int t_productrange) ;
private :
    int code, billno, length, productrange ;
    int dd, mm, yy ;
...
void product :: list_of_item(void)
{
    ...
    cout <<"LIST OF ITEMS" ;
    gotoxy(3,4) ;
    cout <<"ITEM CODE   ITEM NAME   ITEM COST    ITEM PRICE  NO. OF ITEM AVAILABLE" ;
    gotoxy(2,5) ;
    cout <<"***********************************************************" ;
    while (file.read((char *) this, sizeof(product)))
    {
...
{
    int tcode, valid ;
    char ch, t_itemcost[10], t_itemprice[10], t_productrange[10] ;
    tcode = last_code() ;
    tcode++ ;
    do
    {
        ...
        do
        {
            ...
            clreol() ;
            gotoxy(3,25) ;
            cout <<"ENTER NUMBER OF PRODUCTS AVAILABLE IN STOCK TO ADD IN THE MENU" ;
            gotoxy(5,14) ;
            cout <<"Number of item in stock : " ;
            gets(t_productrange) ;
            productrange = atof(t_productrange) ;
            if (t_productrange[0] == '0')
                return ;
            if (productrange < 1 || productrange > 1000)
            {
                valid = 0 ;
                gotoxy(3,24) ;
                cout <<"\7 Range = " <<productrange <<"..1000" ;
                getch() ;
            }
        }
        while (!valid) ;
...
void product :: display_record(int tcode)
{
    ...
    while (file.read((char *) this, sizeof(product)))
    {
        if (itemcode == tcode)
        {
            ...
            gotoxy(5,7) ;
            cout <<"No. of Item Available : "<<productrange ;
            break ;
        }
    }
    file.close() ;
...
void product :: modify_record(int tcode)
{
    ...
    int valid, t_code ;
    char ch, t_itemcost[10], t_itemprice[10], t_itemcode[5], t_productrange[10] ;
    gotoxy(71,2) ;
    cout <<"<0>=Exit" ;
    gotoxy(5,12) ;
    cout <<"Item Code : " ;
    ...
    while (ch == 'Y' && !valid)
    {
        ...
        clreol() ;
        gotoxy(3,25) ;
        cout <<"ENTER NO. OF ITEM AVAILABLE TO ADD IN THE MENU" ;
        gotoxy(5,20) ;
        cout <<"No. of Item Available : " ;
        gets(t_productrange) ;
        productrange = atof(t_productrange) ;
